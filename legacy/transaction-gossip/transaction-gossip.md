...
- Transaction payloads that are being passed around the network are "self validating", meaning that they include both the transaction object and a proof against the sila state adequate to validate `sender.balance` and `sender.nonce` values.
