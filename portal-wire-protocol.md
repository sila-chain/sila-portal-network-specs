...
The Portal wire protocol enables nodes to communicate over the [Node Discovery Protocol v5](https://github.com/sila-chain/sila-devp2p/blob/56a498ee34ee0fb69ffd33dda026d632af4c4048/discv5/discv5-wire.md#talkreq-request-0x05) layer using the `TALKREQ` and `TALKRESP` messages.
...
Each client operates only on one chain (e.g. SilaMainnet, SilaSepolia), against which it validates the content.
...
- `0x500C`: Beacon Chain Network (legacy)
- `0x500D`: Execution Canonical Transaction Index Network (legacy, not implemented)
...
The transmission of data that is too large to fit a single packet is done using [uTP](../assets/sip-7718/bep_0029-rst_post.pdf).

> The Portal wire protocol currently implements uTP over the `TALKREQ/TALKRESP` messages.  Future plans are to move to the [sub-protocol data transmission](https://github.com/sila-chain/sila-devp2p/issues/229) in order to use a protocol native mechanism for establishing packet streams between clients.


## Request - Response Messages

The messages in the protocol are transmitted using the `TALKREQ` and `TALKRESP` messages from the base [Node Discovery Protocol](https://github.com/sila-chain/sila-devp2p/blob/56a498ee34ee0fb69ffd33dda026d632af4c4048/discv5/discv5-wire.md#talkreq-request-0x05).
...
All messages are encoded as an [SSZ Union](https://github.com/sila-chain/consensus-specs/blob/04f5ec595d78c0e3e43794fb7644c18f2584770d/ssz/simple-serialize.md#union) type.
